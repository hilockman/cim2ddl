...
namespace	PRMemDB
{
	...
	public:
		...
		int PRGetBlockSize() const;

		int PRGetTableNum() const;
		int PRGetTableMax(const int nTable) const;
		const char*	PRGetTableName(const int nTable) const;
		const char*	PRGetTableDesp(const int nTable) const;
		int PRGetTableCategory(const int nTable) const;
		int PRGetTableIndex(const char* lpszTableName) const;
		int PRGetTableFieldNum(const int nTable) const;

		int PRGetFieldIndex(const int nTable, const char* lpszFieldName) const;
		...
		int PRGetFieldCategory(const int nTable, const int nField) const;
		int PRGetFieldType(const int nTable, const int nField) const;
		int PRGetFieldLen(const int nTable, const int nField) const;

		int PRGetFieldNameArray(const int nTable, char* lpszRetString) const;
		int PRGetFieldDespArray(const int nTable, char* lpszRetString) const;
		int PRGetFieldCategoryArray(const int nTable, char* lpszRetString) const;
		int PRGetFieldTypeArray(const int nTable, char* lpszRetString) const;
		int PRGetFieldLenArray(const int nTable, char* lpszRetString) const;

		int PRGetFieldEnumNum(const int nTable, const int nField) const;
		int PRGetFieldEnumValue(const int nTable, const int nField, const char* lpszEnumName) const;
		const char*	PRGetFieldEnumString(const int nTable, const int nField, const int nEnumValue) const;
		int PRGetFieldEnumStringArray(const int nTable, const int nField, int& nEnumNum, char lpszEnumArray[][MDB_CHARLEN]) const;

		int PRGetTablePrimaryKeyNum(const int nTable) const;
		int PRGetTablePrimaryKey(const int nTable, const int nRest) const;
		int PRIsFieldPrimaryKey(const int nTable, const int nField);
		...
		void PRMemDBIsland(tagPRBlock* pPRBlock, const unsigned char bRingIslandOnly=1);				//	电岛计算中仅仅判断支路的停运状态，母线的停运状态不考虑。
		void PRTraverseRadial(tagPRBlock* pPRBlock, const int nRadial, int& nBusNum, int* pnBusArray);

		int PRGetFieldCategoryNum() const;
		const char*	PRGetFieldCategoryName(const int nCatrgory) const;
		int PRGetFieldCategoryValue(const char* lpszCatrgory) const;
