...
namespace	PGMemDB
{
	void	PGMemDBAbnorm(tagPGBlock* pPGBlock, const int nLossFlag)
	{
		...
		if (pPGBlock->m_SubstationArray[pPGBlock->m_nRecordNum[PG_SUBSTATION]].nVoltageLevelRange != pPGBlock->m_nRecordNum[PG_VOLTAGELEVEL])
		{
			Log(m_lpszAbnormalLogFile, "请检查 厂站表中的电压指针，可能有厂站的电压漏填，致命错误 \n");
		}
		if (pPGBlock->m_SubstationArray[pPGBlock->m_nRecordNum[PG_SUBSTATION]].nTransformerWindingRange != pPGBlock->m_nRecordNum[PG_TRANSFORMERWINDING])
		{
			Log(m_lpszAbnormalLogFile, "请检查 厂站表中的变压器指针，可能有厂站的变压器有错，致命错误 \n");
		}
		if (pPGBlock->m_SubstationArray[pPGBlock->m_nRecordNum[PG_SUBSTATION]].nRectifierInverterRange != pPGBlock->m_nRecordNum[PG_RECTIFIERINVERTER])
		{
			Log(m_lpszAbnormalLogFile, "请检查 厂站表中的整流逆变器指针，可能有厂站的整流逆变器有错，致命错误 \n");
		}
		if (pPGBlock->m_VoltageLevelArray[pPGBlock->m_nRecordNum[PG_VOLTAGELEVEL]].nConnecivityNodeRange != pPGBlock->m_nRecordNum[PG_CONNECTIVITYNODE])
		{
			Log(m_lpszAbnormalLogFile, "请检查 电压等级和节点表（指针不对），可能有错，致命错误 \n");
		}
		if (pPGBlock->m_VoltageLevelArray[pPGBlock->m_nRecordNum[PG_VOLTAGELEVEL]].nSynchronousMachineRange != pPGBlock->m_nRecordNum[PG_SYNCHRONOUSMACHINE])
		{
			Log(m_lpszAbnormalLogFile, "请检查 电压等级和发电机表（指针不对），可能有错，致命错误 \n");
		}
		if (pPGBlock->m_VoltageLevelArray[pPGBlock->m_nRecordNum[PG_VOLTAGELEVEL]].nEnergyConsumerRange != pPGBlock->m_nRecordNum[PG_ENERGYCONSUMER])
		{
			Log(m_lpszAbnormalLogFile, "请检查 电压等级和负荷表（指针不对），可能有错，致命错误 \n");
		}
		if (pPGBlock->m_VoltageLevelArray[pPGBlock->m_nRecordNum[PG_VOLTAGELEVEL]].nShuntCompensatorRange != pPGBlock->m_nRecordNum[PG_SHUNTCOMPENSATOR])
		{
			Log(m_lpszAbnormalLogFile, "请检查 电压等级和电容、电抗器表（指针不对），可能有错，致命错误 \n");
		}
		if (pPGBlock->m_VoltageLevelArray[pPGBlock->m_nRecordNum[PG_VOLTAGELEVEL]].nBusbarSectionRange != pPGBlock->m_nRecordNum[PG_BUSBARSECTION])
		{
			Log(m_lpszAbnormalLogFile, "请检查 电压等级和母线表（指针不对），可能有错，致命错误 \n");
		}
		if (pPGBlock->m_VoltageLevelArray[pPGBlock->m_nRecordNum[PG_VOLTAGELEVEL]].nBreakerRange != pPGBlock->m_nRecordNum[PG_BREAKER])
		{
			Log(m_lpszAbnormalLogFile, "请检查 电压等级和开关表（指针不对），可能有错，致命错误 \n");
		}
		if (pPGBlock->m_VoltageLevelArray[pPGBlock->m_nRecordNum[PG_VOLTAGELEVEL]].nDisconnectorRange != pPGBlock->m_nRecordNum[PG_DISCONNECTOR])
		{
			Log(m_lpszAbnormalLogFile, "请检查 电压等级和隔离装置表（指针不对），可能有错，致命错误 \n");
		}
		// 	if (pPGBlock->m_VoltageLevelArray[pPGBlock->m_nRecordNum[PG_VOLTAGELEVEL]].pRri != pPGBlock->m_nRecordNum[PG_RECTIFIERINVERTER])
		// 	{
		// 		Log(m_lpszAbnormalLogFile, "请检查 电压等级和整流逆变器表（指针不对），可能有错，致命错误 \n");
		// 	}
...
// 		for (nDev=0; nDev<pPGBlock->m_nRecordNum[PG_ACLINESEGMENT]; nDev++)
// 		{
// 			if (pPGBlock->m_ACLineSegmentArray[nDev].nNodeI < 0 || pPGBlock->m_ACLineSegmentArray[nDev].nNodeJ < 0 || pPGBlock->m_ACLineSegmentArray[nDev].nTopoBusI == 0 || pPGBlock->m_ACLineSegmentArray[nDev].nTopoBusJ == 0)
// 			{
// 				Log(m_lpszAbnormalLogFile, "请检查线路[%s (%s-%s)]信息(拓扑错误 nd, bs): \n", pPGBlock->m_ACLineSegmentArray[nDev].szName, pPGBlock->m_ACLineSegmentArray[nDev].szSubI, pPGBlock->m_ACLineSegmentArray[nDev].szSubJ);
// 			}
// 		}
// 		for (nDev=0; nDev<pPGBlock->m_nRecordNum[PG_TRANSFORMERWINDING]; nDev++)
// 		{
// 			if (pPGBlock->m_TransformerWindingArray[nDev].nNodeI < 0 || pPGBlock->m_TransformerWindingArray[nDev].nNodeJ < 0 || pPGBlock->m_TransformerWindingArray[nDev].nTopoBusI == 0 || pPGBlock->m_TransformerWindingArray[nDev].nTopoBusJ == 0)
// 			{
// 				Log(m_lpszAbnormalLogFile, "请检查变压器[%s-%s]信息(拓扑错误 nd, bs) \n", pPGBlock->m_TransformerWindingArray[nDev].szSub, pPGBlock->m_TransformerWindingArray[nDev].szName);
// 			}
// 		}
// 		for (nDev=0; nDev<pPGBlock->m_nRecordNum[PG_SYNCHRONOUSMACHINE]; nDev++)
// 		{
// 			if (pPGBlock->m_SynchronousMachineArray[nDev].nNode < 0 || pPGBlock->m_SynchronousMachineArray[nDev].nTopoBus == 0)
// 			{
// 				Log(m_lpszAbnormalLogFile, "请检查发电机[%s %s %s]信息(拓扑错误 nd, bs) \n", pPGBlock->m_SynchronousMachineArray[nDev].szSub, pPGBlock->m_SynchronousMachineArray[nDev].szVolt, pPGBlock->m_SynchronousMachineArray[nDev].szName);
// 			}
// 		}
// 		for (nDev=0; nDev<pPGBlock->m_nRecordNum[PG_ENERGYCONSUMER]; nDev++)
// 		{
// 			if (pPGBlock->m_EnergyConsumerArray[nDev].nNode < 0 || pPGBlock->m_EnergyConsumerArray[nDev].nTopoBus == 0)
// 			{
// 				Log(m_lpszAbnormalLogFile, "请检查负荷[%s-%s-%s]信息(拓扑错误 nd, bs) \n", pPGBlock->m_EnergyConsumerArray[nDev].szSub, pPGBlock->m_EnergyConsumerArray[nDev].szVolt, pPGBlock->m_EnergyConsumerArray[nDev].szName);
// 			}
// 		}
// 		for (nDev=0; nDev<pPGBlock->m_nRecordNum[PG_SHUNTCOMPENSATOR]; nDev++)
// 		{
// 			if (pPGBlock->m_ShuntCompensatorArray[nDev].nNode < 0 || pPGBlock->m_ShuntCompensatorArray[nDev].nTopoBus == 0)
// 			{
// 				Log(m_lpszAbnormalLogFile, "请检查补偿[%s-%s-%s]信息(拓扑错误 nd, bs) \n", pPGBlock->m_ShuntCompensatorArray[nDev].szSub, pPGBlock->m_ShuntCompensatorArray[nDev].szVolt, pPGBlock->m_ShuntCompensatorArray[nDev].szName);
// 			}
// 		}
		for (i=0; i<pPGBlock->m_nRecordNum[PG_COMPANY]; i++)
		{
