...
BEGIN_MESSAGE_MAP(CListViewEx, CListView)
	ON_NOTIFY_REFLECT(NM_CUSTOMDRAW, &CListViewEx::OnNMCustomdraw)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONDBLCLK()
	ON_WM_RBUTTONDOWN()
	ON_NOTIFY_REFLECT(LVN_BEGINLABELEDIT, &CListViewEx::OnLvnBeginlabeledit)
	ON_NOTIFY_REFLECT(LVN_ENDLABELEDIT, &CListViewEx::OnLvnEndlabeledit)
	ON_NOTIFY_REFLECT(LVN_COLUMNCLICK, &CListViewEx::OnLvnColumnclick)
	ON_WM_DESTROY()
END_MESSAGE_MAP()

//////////////////////////////////////////////////////////////////////////
// override CListView interfaces
CImageList* CListViewEx::SetImageList( CImageList* pImageList, int nImageListType )
{
	m_nImageListType=nImageListType;
	return CListView::GetListCtrl().SetImageList(pImageList, nImageListType);
}

int CListViewEx::InsertColumn(int nCol, CString strColHead, int nWidth, ColumnType eColType, int nFormat, 
										SortType eSortBy, int nSubItem )
{
	//
	m_mapCol2ColType[nCol]=eColType;
	m_mapCol2Sort[nCol].m_eSortType=eSortBy;
	return CListView::GetListCtrl().InsertColumn(nCol, strColHead, nFormat, nWidth, nSubItem);
}

BOOL CListViewEx::DeleteColumn( int nCol  )
{
	if (!CListView::GetListCtrl().DeleteColumn(nCol))
	{
		return FALSE;
	}
...
BOOL	CListViewEx::DeleteAllItems( )
{
	if (!CListView::GetListCtrl().DeleteAllItems())
	{
		return FALSE;
	}
...
BOOL CListViewEx::DeleteItem( int nItem )
{
	if (!CListView::GetListCtrl().DeleteItem(nItem))
	{
		return FALSE;
	}
...
// public interfaces
CListViewEx::CellIndex CListViewEx::Point2Cell(const CPoint &point)
{
	LVHITTESTINFO lvHitTestInfo;
	CRect rcItem;
	lvHitTestInfo.pt = point;
	if ((GetListCtrl().HitTest(&lvHitTestInfo) >= 0 || GetListCtrl().SubItemHitTest(&lvHitTestInfo) >=0 )&& lvHitTestInfo.iItem >= 0)
	{
...
void CListViewEx::SetColumnType(int nColIndex, ColumnType eColType)
{
...
CListViewEx::ColumnType CListViewEx::GetColumnType(int nColIndex)
{
...
SortType CListViewEx::GetColumnSortBy(int nColIndex)
{
	// insert new if not found
...
void CListViewEx::SetColumnSortBy(int nColIndex, SortType eSortBy)
{
	// init sort map
...
void CListViewEx::SetCellEnabled(int nRow, int nCol, BOOL bEnabled)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol< GetColumnCount());
...
BOOL CListViewEx::GetCellEnabled(int nRow, int nCol)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol< GetColumnCount());
...
void CListViewEx::SetCellImage(int nRow, int nCol, int nImage)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	FindCellData(nRow, nCol).m_nImage=nImage;
}

void CListViewEx::SetCellChecked(int nRow, int nCol, BOOL bChecked)
{
	int nColCnt=GetColumnCount();
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol<nColCnt );
	...
	else if(RadioBox==eColType)
	{
		...
		int nRowCnt=GetListCtrl().GetItemCount();
		CRect rcCell;
...
BOOL CListViewEx::GetCellChecked(int nRow, int nCol)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	return FindCellData(nRow, nCol).GetCheck();
}

int	CListViewEx::GetCellRadioGrp(int nRow, int nCol)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	return FindCellData(nRow, nCol).m_nRadioGroup;
}

void CListViewEx::SetCellRadioGrp(int nRow, int nCol, int nGroupNO)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	FindCellData(nRow, nCol).m_nRadioGroup=nGroupNO;
}

void CListViewEx::SetCellStringList(int nRow, int nCol, CStrList &lstStrings)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol< GetColumnCount());
...
void CListViewEx::AddCellString(int nRow, int nCol, CString &str)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	CStrList &aCellStrList=FindCellData(nRow, nCol).m_lstString;
...
void CListViewEx::SetCellValidChars(int nRow, int nCol, CString &strValidChars)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	FindCellData(nRow, nCol).m_strValidChars=strValidChars;
}

void CListViewEx::SetCellProgressMaxValue(int nRow, int nCol, UINT uMaxValue)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol< GetColumnCount());
...
void CListViewEx::SetCellProgressValue(int nRow, int nCol, UINT uValue)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount() && nCol>=0 && nCol< GetColumnCount());
...
BOOL CListViewEx::IsShowPogressPercent()
{ 
	return m_dwListCtrlExStyle & SHOW_PROGRESS_PERCENT; 
}
void CListViewEx::SetShowProgressPercent(BOOL bShowProgPercent)
{
	if (bShowProgPercent)
...
BOOL CListViewEx::IsSupportSort()
{
	return m_dwListCtrlExStyle & SUPPORT_SORT;
}

void CListViewEx::SetSupportSort(BOOL bSuptSort)
{
	if (bSuptSort)
...
//////////////////////////////////////////////////////////////////////////
BOOL CListViewEx::GetCellRect(CellIndex ix, CRect &rect)
{
	return GetCellRect(ix.first, ix.second, rect);
}

BOOL CListViewEx::GetCellRect(int iRow, int iCol, CRect &rect, int nArea)
{
	ASSERT(0<=iRow && iRow<GetListCtrl().GetItemCount() && iCol>=0 && iCol< GetColumnCount());
	if(iCol)
		return GetListCtrl().GetSubItemRect(iRow, iCol, nArea, rect);

	if(GetColumnCount()== 1)
		return GetListCtrl().GetItemRect(iRow, rect, nArea);

	iCol = 1;
	CRect rCol1;
	if(!GetListCtrl().GetSubItemRect(iRow, iCol, nArea, rCol1))
		return FALSE;

	if(!GetListCtrl().GetItemRect(iRow, rect, nArea))
		return FALSE;
...
BOOL	CListViewEx::GetCheckRect(int iRow, int iCol, CRect &rect)
{
...
BOOL	CListViewEx::GetRowRect(int nRow, CRect &rcRow)
{
	ASSERT(0<=nRow && nRow<GetListCtrl().GetItemCount());
	return GetListCtrl().GetItemRect(nRow, rcRow, LVIR_BOUNDS);
}

BOOL	CListViewEx::GetColRect(int nCol, CRect &rcCol)
{
...
int CListViewEx::GetColumnFmt(int nCol)
{
...
//////////////////////////////////////////////////////////////////////////
// customer draw
void CListViewEx::OnNMCustomdraw(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	case CDDS_ITEMPREPAINT|CDDS_SUBITEM:			// recd when CDRF_NOTIFYSUBITEMDRAW is returned in
		{	
			...
			*pResult = CDRF_DODEFAULT;			
			CString strText = GetListCtrl().GetItemText(iRow, iCol);
			CRect rcCell;
			if (GetCellRect(iRow, iCol, rcCell))
			{
				bool bSelected=GetListCtrl().GetItemState(iRow, LVIS_SELECTED) == LVIS_SELECTED;
...
void CListViewEx::DrawCell(CDC *pDC,  CString &strText, CRect &rcCell, BOOL bSelected, 
...
// draw normal text
void CListViewEx::DrawNormal(CDC *pDC, CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData, int uLvcFmt)
{
...
void CListViewEx::DrawCheckBox(CDC *pDC,  CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData)
{
	COLORREF crText=bSelected?GetSysColor(COLOR_HIGHLIGHTTEXT):GetSysColor(COLOR_WINDOWTEXT);
	COLORREF crTextBkgrnd=bSelected?GetSysColor(COLOR_HIGHLIGHT):GetSysColor(COLOR_WINDOW);
...
void CListViewEx::DrawRadioBox(CDC *pDC,  CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData)
{
	COLORREF crText=bSelected?GetSysColor(COLOR_HIGHLIGHTTEXT):GetSysColor(COLOR_WINDOWTEXT);
	COLORREF crTextBkgrnd=bSelected?GetSysColor(COLOR_HIGHLIGHT):GetSysColor(COLOR_WINDOW);
...
void CListViewEx::DrawComboBox(CDC *pDC,  CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData)
{
	COLORREF crText=bSelected?GetSysColor(COLOR_HIGHLIGHTTEXT):GetSysColor(COLOR_WINDOWTEXT);
	COLORREF crTextBkgrnd=bSelected?GetSysColor(COLOR_HIGHLIGHT):GetSysColor(COLOR_WINDOW);
...
void CListViewEx::DrawEditBox(CDC *pDC,  CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData, int uLvcFmt)
{
	COLORREF crText=bSelected?GetSysColor(COLOR_HIGHLIGHTTEXT):GetSysColor(COLOR_WINDOWTEXT);
	COLORREF crTextBkgrnd=bSelected?GetSysColor(COLOR_HIGHLIGHT):GetSysColor(COLOR_WINDOW);
...
// draw a progress bar
void CListViewEx::DrawProgress(CDC *pDC,  CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData)
{
	COLORREF crText=bSelected?GetSysColor(COLOR_HIGHLIGHTTEXT):GetSysColor(COLOR_WINDOWTEXT);
	COLORREF crTextBkgrnd=bSelected?GetSysColor(COLOR_HIGHLIGHT):GetSysColor(COLOR_WINDOW);
...
//////////////////////////////////////////////////////////////////////////
// kernel data 
int	CListViewEx::GetProgBarSize(const CListViewEx::CellData &cellData, CRect &rcProg, float *pPersent)
{
	float fScale=0.0f;
...
CImageList* CListViewEx::TheImageList()
{
	return GetListCtrl().GetImageList(m_nImageListType);
}

CListViewEx::CellData& CListViewEx::FindCellData(CellIndex ix)
{
	return FindCellData(ix.first, ix.second);
}

CListViewEx::CellData& CListViewEx::FindCellData(int nRow, int nCol)
{
	// init if empty
...
//////////////////////////////////////////////////////////////////////////
// show control
void CListViewEx::ShowCellInPlace(CListViewEx::CellIndex ix, CellType eCellType)
{
	// roll up make the cell be visible
	if (!GetListCtrl().EnsureVisible(ix.first, TRUE))
	{
		return;
	}
	switch(eCellType)
...
void CListViewEx::ShowInPlaceCombo(CellIndex ix)
{
	if (ix.first>=0 && ix.first<GetListCtrl().GetItemCount() && ix.second>=0 && ix.second< GetColumnCount())
	{
		CString strCurSel= GetListCtrl().GetItemText(ix.first, ix.second);
		CRect rcCell;
...
void CListViewEx::ShowInPlaceEdit(CellIndex ix)
{
	if (ix.first>=0 && ix.first<GetListCtrl().GetItemCount() && ix.second>=0 && ix.second< GetColumnCount())
	{
		CString strCurSel= GetListCtrl().GetItemText(ix.first, ix.second);
...
//////////////////////////////////////////////////////////////////////////
// message handlers
void ListCtrlEx::CListViewEx::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	bShouldAction=(ix.first>=0 && ix.second>=0 && ix.first<GetListCtrl().GetItemCount() && ix.second<GetColumnCount());
...
void ListCtrlEx::CListViewEx::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	CWnd* pWnd=NULL;
	bool bShouldAction=false;
	CellIndex ix=Point2Cell(point);
...
void ListCtrlEx::CListViewEx::OnRButtonDown(UINT nFlags, CPoint point)
{
	CWnd* pWnd=NULL;
	bool bShouldAction=false;
	CellIndex ix=Point2Cell(point);
...
void ListCtrlEx::CListViewEx::OnLvnBeginlabeledit(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void ListCtrlEx::CListViewEx::OnLvnEndlabeledit(NMHDR *pNMHDR, LRESULT *pResult)
{
	NMLVDISPINFO *pDispInfo = reinterpret_cast<NMLVDISPINFO*>(pNMHDR);
	// TODO: Add your control notification handler code here
	// Update the item text with the new text
	TRACE("OnLvnEndlabeledit %d.%d %s \n", pDispInfo->item.iItem, pDispInfo->item.iSubItem, pDispInfo->item.pszText);

	GetListCtrl().SetItemText(pDispInfo->item.iItem, pDispInfo->item.iSubItem, pDispInfo->item.pszText);
...
//////////////////////////////////////////////////////////////////////////
// static members
int CALLBACK CListViewEx::CompareFunc(LPARAM lParam1, LPARAM lParam2, 
						 LPARAM lParamSort)
{
	_SortParam_t *pSortParam_t=reinterpret_cast<_SortParam_t*>(lParamSort);
	if (NULL==pSortParam_t)
		return 0;
	CListViewEx *pList=pSortParam_t->m_pTheList;
	ASSERT(pList!=NULL && ::IsWindow(pList->m_hWnd));
	CString strCell1 = pList->GetListCtrl().GetItemText((int)lParam1, pSortParam_t->m_nSortCol);   
	CString strCell2 = pList->GetListCtrl().GetItemText((int)lParam2, pSortParam_t->m_nSortCol);   
	int nResult=0;
	try
	{
...
bool CListViewEx::IsDigitStr(LPCTSTR str)
{
...
int CListViewEx::IntCompare(LPCTSTR strInt1st, LPCTSTR strInt2nd)
{
...
int CListViewEx::StrCompare(LPCTSTR str1st, LPCTSTR str2nd)
{
...
BOOL CListViewEx::DrawGradientRect(CDC *pDC, CRect &rect, COLORREF crTopLeft, COLORREF crBottomRight)
{
	ASSERT(pDC);
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void ListCtrlEx::CListViewEx::OnLvnColumnclick(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMLISTVIEW pNMLV = reinterpret_cast<LPNMLISTVIEW>(pNMHDR);
	if(IsSupportSort())
	{
		vector<DWORD_PTR> vecItemDatas(GetListCtrl().GetItemCount());
		//map<int, int> mapOldRow2NewRow(GetListCtrl().GetItemCount());
		...
		GetListCtrl().SortItems(CompareFunc, (DWORD_PTR)&aSortParam);
...
// vec2StoreData must be initialized as size of "GetListCtrl().GetItemCount()"
void CListViewEx::PreSortItems( vector<DWORD_PTR> &vec2StoreData )
{
	int nRowCnt=GetListCtrl().GetItemCount();
	for (int i=0; i<nRowCnt; ++i)
	{
		vec2StoreData[i]=GetListCtrl().GetItemData(i);
		GetListCtrl().SetItemData(i, (DWORD_PTR)i);
	}
}

// after sort items, re-mapping the cell data here and restore the old item data here
// vec2StoreData must be initialized as size of "GetListCtrl().GetItemCount()" and assigned with old item data
void CListViewEx::PostSortItems( vector<DWORD_PTR> &vec2StoreData )
{
	...
	for (int i=0; i<nRowCnt; ++i)
	{
		int nOldRow=(int)GetListCtrl().GetItemData(i);
		for (int j=0; j<nColCnt; ++j)
		{
			newMap[make_pair(i, j)]=m_mapCell2Data[make_pair(nOldRow, j)];	
		}
		// restore item data
		GetListCtrl().SetItemData(i, (DWORD_PTR)vec2StoreData[nOldRow]);
	}
	m_mapCell2Data.swap(newMap);	
}

void ListCtrlEx::CListViewEx::PreSubclassWindow()
{
	// TODO: Add your specialized code here and/or call the base class
	ASSERT( GetStyle() & LVS_REPORT );	
	CListView::PreSubclassWindow();
	VERIFY(m_ctlSortHead.SubclassWindow(this->GetListCtrl().GetHeaderCtrl()->GetSafeHwnd()) );
}
BOOL	CListViewEx::IsColumnSortAsc(int nColIndex)
{
	// insert new if not found
	ColSortMap::iterator iter=m_mapCol2Sort.find(nColIndex);
	if (m_mapCol2Sort.end()==iter)
...
void CListViewEx::SetColumnSortDirection(int nColIndex, BOOL bIsAsc )
{
	// init sort map
	if (m_mapCol2Sort.empty())
	{
...
void ListCtrlEx::CListViewEx::OnDestroy()
{
	CListView::OnDestroy();
