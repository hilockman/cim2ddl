...
struct QSvgAttributes
{
	...
	QVariantMap extendedAttributes;//other attribute name-->value
};
        ...
        for (int j = 0; j < m_cssAttributes.count(); ++j) {
            ...
            case 'c':
                if (name == QLatin1String("color"))
					...
					extendedAttributes.insert(name.toString(), value.toString());
                break;
...
static bool resolveColor(const QStringRef &colorStr, QColor &color, QSvgHandler *handler)
{
    ...
    switch(colorStrTr.at(0).unicode()) {
        ...
        case 'i':
            ...
            break;
        default:
			color.setRgb(colorStrTr.toInt());//fill="10399959"
			return true;
            break;
    }
                     ...
                     QSvgHandler *handler)
{
    ...
    if (!attributes.stroke.isEmpty() || !attributes.strokeDashArray.isEmpty() || !attributes.strokeDashOffset.isEmpty() || !attributes.strokeLineCap.isEmpty()
		...
		QSvgStrokeStyle *prop = (QSvgStrokeStyle*)(node->styleProperty(QSvgStyleProperty::STROKE));
		if (prop == NULL)// both css style  and  node attribute have the stroke attribute
            prop = new QSvgStrokeStyle;
                      ...
                      QSvgHandler *handler)
{
	...
	if (!fontStyle)
	{
		QSvgFontStyle *fStyle = (QSvgFontStyle*)(node->styleProperty(QSvgStyleProperty::FONT));// both css style  and  node attribute have the font attribute
		if (fStyle)
			fontStyle = fStyle;
		else
...
static void parseExtened(QSvgNode *node,
	const QSvgAttributes &attributes,
	QSvgHandler *)
{
	QMap<QString, QVariant>::const_iterator i = attributes.extendedAttributes.constBegin();
	while (i != attributes.extendedAttributes.constEnd()) {
		node->addExtendedAttribute(i.key(), i.value());
		++i;
	}
}
                       ...
                       QSvgHandler *handler)
{
	...
	parseExtened(node, attributes, handler);
#if 0
...
static QJsonObject parseExtendedNode(QString localName, QSvgNode *parent,
	const QXmlStreamAttributes &attributes,
	QSvgHandler *)
{
	QJsonObject newObj;
	newObj["nodeType"] = localName;
	newObj["parentNodeID"] = parent->nodeId();
	for (int i = 0; i < attributes.size(); i++)
	{
		QStringRef attrName = attributes[i].qualifiedName();
		QStringRef attrValue = attributes[i].value();
		newObj[attrName.toString()] = attrValue.toString();
	}
	return newObj;
}
...
static FactoryMethod findGroupFactory(const QString &name)
{
    ...
    case 's':
		...
		if (ref == QLatin1String("ymbol")) return createGNode;
        break;
    default:
        break;
    }
...
static ParseMethod findUtilFactory(const QString &name)
{
    ...
    case 'h':
        ...
        break;
    case 'm':
       // if (ref == QLatin1String("etadata")) return parseMetadataNode; //to use parseExtenedNode
                               ...
                               const QXmlStreamAttributes &attributes)
{
    ...
    if (FactoryMethod method = findGroupFactory(localName)) {
		...
		parseStyle(node, attributes, this);//先处理属性里的style，然后处理class引用的cssstyle？最后css生效？
#ifndef QT_NO_CSSPARSER
		...
		QJsonObject extendedNode = parseExtendedNode(localName, m_nodes.top(), attributes, this); // extended node 
		if (m_doc)
		{
			if (m_skipNodes.count(Unknown)==1)
			{
				m_doc->addExtendedNode(extendedNode);
			}
			else
			{
				QJsonArray ary = m_doc->ExtendedNode();
				QJsonObject child = extendedNode;
				QJsonObject parent = ary.last().toObject();
				QJsonArray children;
				...
				m_doc->replaceExtendedNode(ary.size() - 1, parent);
			}
		}

		//rdfid add to ExtendedAttribute, fast then search metadata in extendedNode
		if (localName == "CN_Ref" || localName == "GLink_Ref"){
            QString id = attributes.value("ObjectID").toString();

            if (id.isEmpty())
                return true;
            QStringList lst=m_nodes.top()->extendedAttribute("link").toStringList();
            lst.append(id);
            m_nodes.top()->addExtendedAttribute("link", lst);

        }
        else if (localName == "PSR_Ref" || localName == "Meas_Ref")
		{
			QString attrName = QSvgTinyDocument::m_psr2rdfidMap.value(localName).toString();
			if (attrName.isEmpty())
				attrName = "ObjectID";
			for (int i = 0; i<attributes.count();i++)
			{
				QString attr = attributes[i].name().toString();
				QString val = attributes[i].value().toString();
				if (val.isEmpty())
					continue;

				if (attr == attrName)
					m_nodes.top()->addExtendedAttribute(localName, val);//PSR_Ref/Meas_Ref---rdfid
				else
					m_nodes.top()->addExtendedAttribute(attr, val);
			}
		}
        else if(localName =="Layer_Ref")
        {
            QString rdfID = attributes.value("ObjectName").toString();
            if (rdfID.isEmpty())
                return true;
            m_nodes.top()->addExtendedAttribute("Layer_Ref", rdfID);
        }
        return true;
    }
...
bool QSvgHandler::characters(const QStringRef &str)
{
    ...
    if (m_inStyle) {
		...
		//处理css 如 .lkv15.75 {fill:none;stroke:rgb(0,128,0)} 其中15.75的.引起解析异常 改为-
		QStringList lst = css.split("{");
		for (int i = 0; i<lst.size();i++)
		{
			QString cssStr = lst[i];
			while (cssStr.count('.') > 1)
			{
				cssStr = cssStr.replace(cssStr.lastIndexOf("."), 1, "-");
			}
			lst[i] = cssStr;
		}
		css = lst.join("{");
